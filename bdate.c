...
/* static TValue * primitive_Date_utc(bean_State * B, TValue * thisVal UNUSED, TValue * args UNUSED, int argc UNUSED) { */
/*   TValue * ret = malloc(sizeof(TValue)); */
/*   Date * date = malloc(sizeof(Date)); */
/*   time_t t = time(NULL); */
/*   date->tm = gmtime(&t); */
...
static TValue * primitive_Date_getTimezoneOffset(bean_State * B, TValue * thisVal, TValue * args UNUSED, int argc UNUSED) {
  ...
  Date * date = datevalue(thisVal);
  time_t time = date -> time;
  localtime(&time); // Call the tzset to set the timezone from environment variable
  setnvalue(ret, timezone);
  return ret;
}
